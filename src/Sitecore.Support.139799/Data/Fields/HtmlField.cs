...
{
  public class HtmlField : CustomField
  {
    // Methods
    ...
    private static string MediaEvaluator(Match match)
    {
      ...
      {
        input = Regex.Replace(input, "src=\"([^\"]*)\"", new MatchEvaluator(HtmlField.MediaEvaluator2), RegexOptions.Singleline | RegexOptions.IgnoreCase);
      }
      ...
      {
        input = Regex.Replace(input, @"src=([^\s]*)", new MatchEvaluator(HtmlField.MediaEvaluator2), RegexOptions.Singleline | RegexOptions.IgnoreCase);
      }
    ...
    public static implicit operator HtmlField(Field field)
    {
      if (field != null)
      {
    ...
    {
      Assert.ArgumentNotNull(html, "html");
      html = Regex.Replace(html, "<img([^>]*)>", new MatchEvaluator(HtmlField.MediaEvaluator), RegexOptions.Singleline | RegexOptions.IgnoreCase);
      html = Regex.Replace(html, "<a([^>]*)>", new MatchEvaluator(HtmlField.MediaEvaluator), RegexOptions.Singleline | RegexOptions.IgnoreCase);
